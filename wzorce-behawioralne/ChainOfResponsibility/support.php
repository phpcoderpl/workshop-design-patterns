...
class Helpdesk extends AbstractHandler
{
  ...
  {
    if ($request === "software") {
      return "Helpdesk onsługuje zgłoszenie dotyczące oprogramowania.\n";
    }
...
class SystemAdmin extends AbstractHandler
{
  ...
  {
    if ($request === "network") {
      return "SystemAdmin onsługuje zgłoszenie dotyczące sieci.\n";
    }
...
class Developer extends AbstractHandler
{
  public function handle(string $request): ?string
  {
    if ($request === "system_bug") {
      return "Developer onsługuje zgłoszenie dotyczące błędu systemu.\n";
    }
